...
#### 4. Classfication experiments
 ...
 평가 결과는 아래와 같다. 먼저, LRN의 사용유무에 따라 성능에 차이가 없음이 확인되어 더이상 사용하지 않는다.
  ...
  - training시 scale jittering $$(S \in [256;512])$$ 했을 때 fixed scale로 했을 때$$(S=256 or S=384)$$보다 성능이 좋게 나왔으며 multi-scale image statistics를 capture하는데 도움을 줬을 것으로 생각된다.
 ...
 성능이 향상된 것을 확인할 수 있다. 평가 결과는 아래 표에 나타나있다.
